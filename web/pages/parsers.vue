<template>
  <div class="processors-page">
    <div class="page-header">
      <h2>⚙️ Processor Configuration</h2>
      <p>
        Manage processor settings, dependencies, and create custom
        configurations
      </p>
    </div>
    ...
    <div v-else>
      ...
      <!-- Processor Configurations List -->
      <div class="processors-list">
        <div class="section-header">
          <h3>Current Processor Configurations</h3>
          ...
          >
            <div class="processor-header">
              <div class="processor-title">
                ...
                <span class="processor-name">{{ config.name }}</span>
              </div>
        ...
        <div class="modal-content" @click.stop>
          ...
          <form @submit.prevent="saveConfig" class="config-form">
            <div class="form-group">
              ...
              >
                <option value="">Select a processor implementation...</option>
                <option
                ...
                >
                  {{ processor.name }} ({{
                    processor.inputExtensions.join(", ")
                  }}
                  → {{ processor.outputExt }})
                </option>
              </select>
...
async function loadConfigs() {
  try {
    loading.value = true;
    const response = await $fetch("/api/processor-configs");
    configs.value = response.configs;
    validation.value = response.validation;
  } catch (error) {
    console.error("Failed to load processor configs:", error);
  } finally {
    loading.value = false;
  }
}
...
async function loadAvailableProcessors() {
  try {
    const response = await $fetch("/api/available-processors");
    availableProcessors.value = response.processors;
  } catch (error) {
...
function onProcessorImplementationChange() {
  ...
  if (selectedProcessor && !editingConfig.value) {
    // Pre-populate form with processor defaults
    formData.value.outputExt = selectedProcessor.outputExt;
    inputExtensionsText.value = selectedProcessor.inputExtensions.join(", ");
    dependsOnText.value = selectedProcessor.dependsOn.join(", ");

    // Generate a suggested config name
    if (!formData.value.name) {
      formData.value.name = `auto-${selectedProcessor.name}`;
    }
    if (!formData.value.displayName) {
      formData.value.displayName = `Auto ${selectedProcessor.name.charAt(0).toUpperCase() + selectedProcessor.name.slice(1)}`;
    }
  }
}
...
async function saveConfig() {
  try {
    ...
    await $fetch("/api/processor-configs", {
      method: "POST",
      body: configData,
    });
...
async function deleteConfig(config) {
  if (
    !confirm(
      `Are you sure you want to delete the processor configuration "${config.displayName}"? This action cannot be undone.`
    )
  ) {
    return;
  }
